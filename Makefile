...
# docker-build builds the Seam Core operator image (distroless, linux/amd64).
docker-build:
	docker build \
...
# docker-push pushes the already-built Seam Core image to the registry.
docker-push:
	docker push $(IMAGE_REGISTRY)/$(IMAGE_NAME):$(TAG)

# lint-images verifies the Seam Core image exists in the local OCI registry.
lint-images:
